...
//		new Bird(); //추상클래스는 객체 생성 못함
		Penguin p = new Penguin(); //p=참조변수(객체)
		p.swim(); //멤버5개 (life,wing,walk(),eat(),swimg())중 수영한다.
		
		Bird p2 = new Penguin();//upcasting
//		p2.swim(); //업캐스팅은 슈퍼클래스의 멤버만 접근 가능(오류발생)
		p2.walk();
		Penguin p3 = (Penguin) p2; //downcasting
		p3.swim();//다운캐스팅은 모든 멤버 접근 가능
		
		Bird d = new Duck(); //upcasting 객체생성
...
//		new Swim2(); //인터페이스도 객체 생성 못한다.
		Swim2 s = new Penguin(); //인터페이스를 참조변수 선언
		s.swim(); //penhuins swim.
		
//		인터페이스가 인터페이스 배열2개를 가리킨다.
//		객체배열과 형태가 비슷
//		s2[0],sw[1]
